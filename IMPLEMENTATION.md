...
### Gate 1 — decisions already supplied by the project owner, recovered

A full decision-assimilation matrix was built from
`docs/audits/rio-resilience/phase-v10-f1a-revenue-governance-decision-register.json`,
its companion packet, `docs/finance/dre-finance-confirmation-register.json`, and a
read-only inspection of the pre-existing stash (`stash@{0}`, "WIP rio-scenario-
resilience supporting data" — not popped, not applied, still present). See
`docs/audits/rio-resilience/phase-v10-rc2-gate1-decision-assimilation-matrix.md` for
the full matrix with file:line citations. Summary:

- **D-R1, D-R2, D-R3, D-R4, D-R7, D-R8: approved by the project owner and already
  encoded consistently** in `fopagEngine.ts`, `payrollGrowth.ts`, `receitaEngine.ts`/
  `tuitionGrowth.ts`, `dreRevenueDriverSourceData.ts`, `reajusteDespesasGrowth.ts`/
  `dreLineItemMap.ts`, and `openingPackageOccupancySourceData.ts`.
- **The Early Years staffing rule (1 section = 1 teaching lead + 1 learning assistant
  + 1 learning monitor) is approved** ("approved v1, Phase 8H.1, project owner Luciana
  Polonen, 2026-06-03") and already implemented (`payrollAdapter.ts:344-349`). It does
  not extend to LS (1 lead + 1 assistant, no monitor — a distinct, also-approved rule)
  or to MS/HS, which remain governed by separate, only partially reconciled evidence
  (F06, below).
- **Payroll salary escalation (5.9%/yr from 2029, v10 PnL row 12 "Dissídio"), benefits
  escalation (10%/yr from 2029, row 13), and encargos (48.5% of salary) are approved
  and fully implemented** (Phase V10-P1/V10-P1.1, above, `IMPLEMENTATION.md:4058-4285`,
  validated `validate:v10-p1` 58/58, including the assertion
  `fopagOutput.calculationReady === true`). This corrects the framing of the paragraph
  immediately above this section: **the payroll calculation is ready.** What remains
  genuinely unwired is cross-tab UI state (Gate 3, below), not missing salary/dissídio/
  benefits/encargos governance.
- **The `payrollRoleCostSourceData.ts:12` per-record `calculationReady: false` field
  the paragraph above cites is Phase 13H legacy** (`payrollAdapter.ts:160-164`,
  2026-06-09) — explicitly documented in-repo as superseded by
  `FopagEngineOutput.calculationReady`, computed dynamically per scenario in
  `fopagEngine.ts`. `fopagEngine.ts` never reads `adapterOutput.calculationReady` or
  `adapterOutput.fopagCalculationReady`. Citing the static per-record flag as the live
  blocker for "grade-level staffing completeness" was itself a stale-flag
  misclassification of exactly the kind this phase's directive named. See
  `scripts/validate-v10-rc2-gate2-governance.ts`.
- **Genuinely unresolved, confirmed still open, no invented values:** D-R5
  (`desconto_metodo` re-verification), D-R6/F03 (base tuition rate source — remains
  `screenshot_transcription_based`), F05 (2028 `t1_g3` enrollment: engine 228 vs.
  workbook ~246 — a distinct question from D-R8's `t1_g4` 258/259), and F06
  (instructional-capacity vs. FOPAG MS/HS staffing assumptions — a recovered stash
  document, `payrollStaffingRuleSourceTrace.md`, Phase 8B 2026-06-03, shows a *third*,
  also-unreconciled MS count, g6=3/g7=4/g8=3, distinct from both the Phase 15H.2
  9-educator model and the payroll adapter's own fixed-FTE table).
- **Recovered from the stash and newly committed** (never previously tracked):
  `docs/audits/rio-resilience/phase-v10-rc2-recovered-finance-convention-decisions.md`
  — a 2026-06-03 project-owner-confirmed set of Receita v1 conventions (tuition basis,
  discount-application order, 2029 escalation start, contracted-student enrollment
  measure, full-year 2028 treatment, Receita v1 scope) that were approved but never
  committed to any branch. One item (§2.6, the 8% escalation rate) is superseded by
  the later, explicit D-R2 (5.9%/6.0%, v10-sourced) and is recorded as historical only.

### Gate 2 — governance record corrections

`docs/finance/dre-finance-confirmation-register.json` items **F01** and **F04**
described stale runtime behavior (F01: "reajuste term omitted" — closed by D-R7; F04:
a 20/17/15/12.5 schedule — superseded first by the current D-R3 25/20/20/18/15×3/
12.5×3 schedule). Both are corrected in place with `supersededBy` pointers to the
superseding decision, `approvedFormula`/`approvedValue` populated from that decision,
and `decisionStatus` left `"open"` — only the Finance-signed provenance sign-off
remains outstanding for either item, not the formula or value. `F02` was already
correctly marked `resolved` and required no change. The register's
`enrollmentDocumentation.declarationNote` referenced a scenario id, `"intermediario"`,
that does not exist in the live `OccupancyScenarioId` enum (`conservador`/`base`/
`otimista`); corrected to `"base"` with an explicit note that `"intermediario"` is a
retired legacy alias, deterministically normalized by
`normalizeOccupancyScenarioId()`/`parseOccupancyScenarioId()`
(`openingPackageOccupancySourceDataContract.ts:109-135`) — not a distinct live
scenario.

`scripts/validate-v10-rc2-gate2-governance.ts` (new) proves deterministically: no
F1A decision is simultaneously `approved_by_project_owner*` and `pending`; every
non-null `selectedOption` references a declared alternative; every finance-register
open item carrying `approvedFormula`/`approvedValue` names the F1A decision that
supersedes it, and that decision is itself actually approved; source-file spot checks
for every decision classified `encoded_consistently` in the Gate 1 matrix;
`"intermediario"` does not control any live calculation branch outside its own
normalization contract, and normalizes deterministically to `"base"`; and
`fopagEngine.ts` does not read either Phase-13H-legacy static readiness field.
...
`q5_zero_unresolved_visible_strings_app_wide`, requires zero reachable
(live/imported) visible-string literals outside translated/i18n-flagged files; the
mechanical AST inventory (`validate-v10-x2t-visible-string-inventory.ts`) currently
finds 2772. This is genuine, reproducible, disclosed production incompleteness — a
whole-application translation-closure gap (`OfferScenariosTab.tsx`,
`MiddleSchoolTab.tsx`, `HighSchoolTab.tsx`, and others remain untranslated) named as
explicit, unchanged scope across multiple prior phases, not introduced by V10-RC1B/
RC1B.1 or by this phase. The validator's own header states "Do not weaken existing
checks" and `q18_check_suite_did_not_shrink` enforces a floor of 83 (current 101) —
weakening `q5` to a nonzero ceiling would itself be a governance regression of the
same kind Gate 2 exists to prevent. **Disposition: keep the check as written; do not
attempt the i18n sweep under this phase's scope (it is a separate, bounded,
whole-application task, not a governance/scenario-contract task); report it as a
named, out-of-scope release blocker in the final return, not as a re-blessed
exception.** This phase's own obligation is narrower: every new string introduced by
Gates 3–5 below is added through `t()` with keys present in both `pt-BR.ts` and
...
**Governed counselor ramp at this point in the session: `{2028: 2, 2032: 3}`**
(direct product-owner correction, live in-session, 2026-07-30) — superseding
both the RC2.4-era code value (`{2028: 3, 2031: 4}`) and the RC2.4-recorded
verbal statement (2032 step year only, headcount values unspecified at the
time). **This value was itself superseded later the same session — see
"Second Counselor ramp correction" below, which adds a 2033 step to HC 4.**
Not "final" as originally labeled here; left unedited above as an accurate
record of what was true at this point in the session. Verified:
...
### Checkpoint commit and push (2026-07-30, continued session)

Per explicit product-owner authorization ("PRODUCT-OWNER DECISION —
V10-RC2.5 FROZEN EXPORT FILES" checkpoint follow-up: "The product owner now
authorizes a checkpoint commit and branch push. This does not classify
RC2.5 as release-ready and does not authorize a push to main."), all 33
bounded RC2.5 files (28 modified + 5 untracked, classified in full — no
unrelated or uncertain files existed) were committed to a new branch
`v10-rc2-5-checkpoint`, created from `44697b0`, and pushed to `origin`.
Checkpoint commit: `d2f609bc9f089aee6d110f324963e1e1fdabc8d1` — "V10-RC2.5
checkpoint: unify staffing tiers and payroll exports". `main`/`origin/main`
confirmed unchanged at `44697b0` before and after the push; the pre-existing
stash (`stash@{0}`) untouched; no force-push. Bilingual browser QA remained
**INCOMPLETE** (unchanged from the qualified Browser QA note above) and RC2.5
acceptance remained **HOLD** — the checkpoint is a durable snapshot, not a
release-readiness upgrade.

### Second Counselor ramp correction (2026-07-30, continued session, on `v10-rc2-5-checkpoint`)

A second, same-day product-owner correction revised the governed Counselor
ramp again: **`{2028: 2, 2032: 3, 2033: 4}`** — a fourth Counselor becomes
active in 2033, superseding the `{2028: 2, 2032: 3}` value committed in the
checkpoint above. Required regression years verified end-to-end (source →
`LEADERSHIP_CONFIG` → `payrollRoleCostSourceData.ts` → `calculateFopag()` →
`calculateDre()` → both live exports): 2028 HC=2, 2031 HC=2, 2032 HC=3, 2033
HC=4, 2037 HC=4 — all confirmed via `validate:v10-rc2-5-gate7` assertions
36-42 (rewritten for the new ramp and regression-year set), 50/50 pass.

**Canonical source and consumers updated:** `src/constants/leadership.ts`
(`hc([[2028, 2], [2032, 3], [2033, 4]])`), `payrollRoleCostSourceData.ts`
(`headcountProgression` + `sourceNotes`), `orgDesignPayrollActivation.ts`
(`headcountSource` + `sourceNotes`), `en-US.ts`/`pt-BR.ts` (`"Counselors
(2→4)"` / `"Conselheiros (2→4)"`, replacing `(2→3)`),
`validate-v10-rc2-5-gate7-shared-tier-workflow.ts` (assertions 36-42:
expected-HC array, `headcountProgression` literal, export regression-year
set expanded from {2031, 2032} to {2028, 2031, 2032, 2033, 2037} per the
product-owner's required regression years). `docs/audits/rio-resilience/
phase-v10-rc2-gate8-coverage-matrix.json` was not hand-edited — it is
mechanically regenerated by `validate-v10-rc2-gate8-coverage-matrix.ts`
(confirmed in this file's own prior "Note on working-tree inventory"
section); re-running that script picked up the new ramp automatically
(900/900 cells pass; `payrollTotalPayroll` values for years ≥2033 changed to
reflect the fourth Counselor's cost, years <2033 unchanged).

**Single aggregate record per year preserved, not four duplicates:** the
correction is a single additional breakpoint in one `hc()` progression array
on one `LEADERSHIP_CONFIG` entry — the same mechanism already used for the
2032 step. `calculateFopag()` still produces exactly one `roleId ===
"counselor"` record per year (gate7 assertion 39, re-verified unchanged)
and no additional summary/rollup record was introduced anywhere in the
payroll pipeline.

**Fourth Counselor's division/title: not inferred, flagged as an unresolved
governance question.** This was the most substantive part of the fix.
`src/features/rio-scenario-resilience/model/executiveOrgDesignModel.ts`'s
Executive Org Design tree renders Counselor headcount as three independent,
pre-existing pieces: a hardcoded HC=1 "EY Counselor" node, a hardcoded HC=1
"LS Counselor" node (both via `getCounselorHeadcount()`, unrelated to the
LEADERSHIP_CONFIG ramp), and an "MS Counselor" node that read the ramp
dynamically as `headcount[year] - headcount[2028]` via
`getIncrementalExistingRoleHeadcount("counselor", 2028, year, ...)`. That
generic incremental calculation, if left unchanged, would have silently
folded *both* the newly-added fourth Counselor's headcount and the
originally-established third Counselor's headcount into the "MS Counselor"
node (incremental value 2 from 2033 onward) — implying an MS attribution for
the fourth Counselor that no source in this repository establishes. (A
separate, pre-existing, unrelated "HS Counselor" node, hardcoded HC=1 and
explicitly documented in `orgDesignLogic.md` as a "user-authorized canonical
Executive Org Design rule" that "does not modify payroll... logic," was left
untouched — it predates this correction and is deliberately not reconciled
with the payroll aggregate.)

Repository evidence (the pre-existing node's id/label/mechanism, and the
RC2.4 Gate 6 record's "3rd counselor" framing) establishes only that the
*first* incremental unit above the 2028 baseline is MS-attributed. There is
no equivalent evidence for a second incremental unit. `getIncrementalExistingRoleHeadcount`
(now unused and removed) was replaced with two purpose-built functions:
`getEstablishedCounselorIncrementHeadcount` (caps the MS-attributed
increment at 1, so the node never silently absorbs a second unit) and
`getUnresolvedCounselorIncrementHeadcount` (returns whatever remains above
that established unit — currently 1, from 2033 onward). When the unresolved
remainder is greater than zero, a new sibling node is added: id
`counselor-unresolved-allocation`, label "Counselor (allocation pending)",
badge "Unresolved governance question", `headcountStatus: "source-backed"`
(the number itself *is* source-backed — only its division/title is
unresolved) with a note stating explicitly that no source in this repository
establishes which division or title the additional headcount belongs to.
This is data-driven, using the tree renderer's existing generic
badge/status rendering (`ExecutiveOrgDesignTab.tsx`'s `HeadcountBadge`/
`TreeNode`) — no new UI code was needed.

Reconciliation verified by hand for all required regression years (EY=1 +
LS=1 + MS-established + unresolved, excluding the separate, pre-existing,
undocumented-by-this-correction HS Counselor artifact which only applies
from 2034): 2028/2031 → 1+1+0+0=2; 2032 → 1+1+1+0=3; 2033/2037 →
1+1+1+1=4 — matching the governed aggregate at every required regression
year.

**Validation:** `npx tsc --noEmit` clean; `npm run build` clean (`vite
build`, same pre-existing >500kB chunk warning); `npm run
validate:v10-rc2-5-gate7` 50/50 pass; `npm run validate:v10-x2t` 109/110
pass (only the pre-existing, unrelated `q5_zero_unresolved_visible_strings_app_wide`
now fails — the `no_drift__dreScenarioWorkbook.ts` failure that was present
before the checkpoint commit is now resolved, since `HEAD` itself contains
that file's committed state); `npm run validate:v10-rc2-gate8` 900/900
cells pass after regeneration; `git diff --check` clean.

This correction was made directly on the `v10-rc2-5-checkpoint` branch,
after the checkpoint commit/push above. It has not yet been committed or
pushed as of this note — per the standing instruction to only commit/push
when explicitly asked, this second correction awaits separate authorization
before it is committed.

### Third correction pass (2026-07-30, continued session): user review of leadership.ts/orgDesignPayrollActivation.ts surfaces four items

After reviewing the condensed views of `leadership.ts` and
`orgDesignPayrollActivation.ts` provided this session, the product owner
reported four "persisting mistakes." Each was independently diagnosed
against the live code before any change was made (per the advisor's
guidance: confirm scope, verify blast radius, order risky work last).

**1. MS ramp — confirmed correct, no change.** Stated as `3-7-9`; current
`MS_FTE_BY_GRADE = { g6: 3, g7: 4, g8: 2 }` already produces cumulative
3→7→9 as grades open in sequence. Product owner confirmed no change needed.

**2. HS ramp — corrected.** Stated as `4-7-9-11`; current
`HS_FTE_BY_GRADE` was `{ g9: 4, g10: 2, g11: 3, g12: 2 }` (cumulative
4→6→9→11 — a mismatch at the g10 step). Corrected to `{ g9: 4, g10: 3,
g11: 2, g12: 2 }` (cumulative 4→7→9→11), per the product owner's confirmed
disposition: swap g10/g11, same total (11). Before editing, verified (per
advisor guidance) that `msHsStaffingReadiness.ts`'s
`HIGH_SCHOOL_CANONICAL_FULL_MODEL` (`g9:4,g10:0,g11:3,g12:3`) is a
deliberately separate, already-governed "instructional planning capacity"
reference — explicitly documented as "comparison values only," "does not
wire payroll totals" — and left untouched; it is not the same duplication
class as the leadership.ts/payrollRoleCostSourceData.ts pattern discussed
earlier this session, and changing it was not requested. Also verified the
V10-RC2.4A dormant safety net (`HS_GRADE_ATTRIBUTABLE_SUM !==
HS_VALIDATED_ENVELOPE_TOTAL`) gates on the sum (unchanged at 11), not
individual grade values, so it remains dormant after the swap.
Updated: `payrollAdapter.ts` (`HS_FTE_BY_GRADE` constant, its runtime
diagnostic message, and all narrative comments — historical V10-RC2.4A
rationale left unedited with a superseding pointer added, not rewritten);
`orgDesignPayrollActivation.ts` (four citation strings: the top-level
`baselineActivationRule`, the hs_pool record's `sourceNotes`, and the
"High School Educators" record's `headcountSource`/`allocationModelSource`/
`sourceNotes`). `validate-v10-rc2-5-gate7-shared-tier-workflow.ts` assertion
13 updated to the new per-grade values; two new assertions added (51-52)
asserting the cumulative sequences `[4,7,9,11]` (HS) and `[3,7,9]` (MS)
directly, computed from the live constants — a permanent guard against this
exact class of "corrected multiple times, different values each time"
drift, since the per-grade breakdown alone (assertions 13-14) doesn't
capture what the product owner actually states and checks.

**3. After School — two-part correction (label, then role family).**
First reported as "should have been taken out"; on investigation, the role
was live and correctly staffed (1 FTE from 2028, FOPAG_DIRETO) — the actual
defect was a stale `roleName`. Traced precisely: `payrollAdapter.ts` sets
`roleName: rec.sourceRoleLabel` for every baseline record, and
`payrollRoleCostSourceData.ts`'s `after_school` record had
`sourceRoleLabel: "After School Educator"`, diverging from
`leadership.ts`'s actual role label `"After School Coordinator"` — this is
the same Layer-1-vs-Layer-2 drift class discussed earlier this session,
newly discovered for a second role. Per the product owner's explicit
instruction, the `after_school_coordinator` extension/alias record (a
distinct, real org-design classification concept, referenced from
`orgDesignScenarioExtensions.ts`, `orgDesignLogic.md`, and
`OrgDesignPanel.tsx`, and counted in `extensionRoleCount: 13`) was **not**
deleted — only the two label sites feeding the live `roleName` were fixed:
`payrollRoleCostSourceData.ts:sourceRoleLabel` and
`orgDesignPayrollActivation.ts`'s baseline record `roleName`. Remaining ~10
occurrences of "After School Educator" (OfferScenarios feature i18n keys
treating Educator-vs-Coordinator equivalence as an open, unresolved
validation question; a separate pre-existing FOLHA_DIRETA/FOPAG_DIRETO
mismatch in `en-US.ts`'s `payrollNoteFolha`; docs/AGENTS.md prose) were
surfaced, not edited — rewriting them would assert a resolution the product
owner didn't give.

Mid-fix, the product owner added a second, larger correction: **After
School Coordinator is in the Leadership role family, not Specialists.**
Moved: `leadership.ts` (`role("after_school", ...)` moved from
`SPECIALISTS_CONFIG` to `LEADERSHIP_CONFIG`, layer D→B, dropping the
Specialists-only `regime` tag); `payrollRoleCostSourceData.ts` (record moved
from the Specialists section to the Leadership section, `roleFamily:
"Specialists"` → `"Leadership"`, `layer: "D"` → `"B"`, `regime: "shared
specialist"` → `null`, `sourceSheetOrTab`/`sourceRowOrCellRef` updated to
`LEADERSHIP_CONFIG[8]`); `orgDesignPayrollActivation.ts` (record moved from
the Specialists baseline section to the Leadership baseline section,
citations switched from `SPECIALISTS_HC_SOURCE`/`SPECIALISTS_COST_SOURCE`
to `LEADERSHIP_HC_SOURCE`/`LEADERSHIP_COST_SOURCE`). `allocationModel`
(FOPAG_DIRETO) was deliberately left unchanged — the product owner's
instruction was about role-family classification, not allocation model, and
`baselineRoleSourceType(roleFamily)`/`baselineCostSourceId(roleFamily)` in
`payrollAdapter.ts` derive `roleSourceType`/`costSourceId` from `roleFamily`
automatically, so this one-field change (per file) is what actually moves
the role's live classification — confirmed by reading those two functions
before editing. Remaining `SPECIALISTS_CONFIG` roles (`arts`, `body`,
`music`, `led`) renumbered `SPECIALISTS_CONFIG[0..3]` in
`payrollRoleCostSourceData.ts` to stay accurate after the removal.
`sourceCategoryTotals`/`extractionBoundaries` counts updated:
`leadershipRolesLayerB` 8→9, `specialistRolesLayerD` 6→4 (also reflecting
the hs_pool deletion below), `totalRecords` 33→32.

**4. HS Educator Pool — deleted.** Confirmed before deletion: `hs_pool`
already cost nothing (`payrollAdapter.ts` explicitly skipped
`normalizedRoleId === "hs_pool"` with an `excluded_role` diagnostic,
`excluded_from_v1`, Phase 8C) — dead config, not a live double-count.
Verified the two contract files (`payrollRoleCostSourceDataContract.ts`,
`orgDesignPayrollActivationContract.ts`) type their role-count fields as
plain `number`, not literal-pinned types, so deletion does not require a
contract-level edit. Deleted from: `leadership.ts` (`SPECIALISTS_CONFIG`
entry removed); `payrollRoleCostSourceData.ts` (record removed,
`sourceCategoryTotals`/`extractionBoundaries` updated as above);
`orgDesignPayrollActivation.ts` (record removed, `baselineRoleCount` 26→25,
`baselineActivationRule` narrative updated — "All 25 non-excluded" became
"All 25," since no baseline role remains excluded). The now-dead exclusion
mechanism was removed, not left as stale cruft: `payrollAdapter.ts`'s
`if (rec.normalizedRoleId === "hs_pool") { ...continue; }` skip block
deleted (nothing left to skip); the now-unused `"excluded_role"`
`PayrollAdapterDiagnosticType` member removed from
`payrollAdapterContract.ts` (confirmed zero other usages first).
`fopagEngine.ts`'s two comments describing the exclusion updated to
describe the deletion instead. `validate-v10-p1.ts`'s illustrative comment
(citing `hs_pool 2034` as an example of a late-activating role) updated to
drop the now-nonexistent example. `src/lib/payroll/presenters.ts`'s
`rolePlanningMap` still has stale `after_school`/`hs_pool` entries — left
untouched after confirming (via `orgDesignExportWorkbookBuilder.ts`'s own
comment: "the dead src/lib/payroll/exportXlsx.ts is not imported or reused")
that this map is documented dead code, not consumed by any live export.
`msHsStaffingReadiness.ts`'s `LEGACY_HS_POOL_EVIDENCE` citation (describing
that hs_pool historically existed and needed exclusion) left untouched — it
is historical evidence tracking, still true as a historical fact, same
treatment as the dated audit `.md` files elsewhere in this document.

**Validation:** `npx tsc --noEmit` clean; `npm run build` clean (1,205
modules, same pre-existing chunk warning); `npm run validate:v10-rc2-5-gate7`
52/52 pass (2 new assertions); `npm run validate:v10-x2t` 109/110 pass
(same single pre-existing, unrelated failure as the prior pass); `npm run
validate:v10-p1` 58/58 pass, unaffected; `npm run validate:v10-rc2-gate8`
900/900 cells pass after regeneration — diff confirmed narrow and
attributable: 495/900 cells' `payrollTotalPayroll` changed, every one in
years 2032-2037 (where HS is ramping) for both opening packages, zero
changes in 2028-2031 — confirming the After School role-family move and the
hs_pool deletion produced no cost change (as expected, pure reclassification
and dead-config removal) and that the HS swap correctly propagates for
every year it applies (neither package reaches full four-grade HS maturity
within the simulated 2028-2037 window, so the g10/g11 split matters through
the whole horizon once it first diverges, not just a single transition
year). `git diff --check` clean.

Not committed or pushed, per the standing instruction — this third
correction pass, like the second, awaits separate authorization.

### Fourth pass (2026-07-30, continued session): Executive Org Design tree / payroll engine parity gate

While tracing where the Payroll tab, Org Design tab, and DRE tab each get
their data (a direct follow-up question this session), a live version of
the same duplication class was found: `ExecutiveOrgDesignTab.tsx` renders
two things from two different pathways on one page. The org-chart tree
(`buildExecutiveOrgDesignTree()`) reads `LEADERSHIP_CONFIG`/
`BACKOFFICE_CONFIG`/`SPECIALISTS_CONFIG` directly via
`getExistingRoleHeadcount()` — it never calls `calculateFopag()`. The
Role-Level Headcount table on the same page (`buildOrgDesignHcTable()`)
calls `calculateFopag()`, the full adapter pipeline, reading
`payrollRoleCostSourceData.ts`. Nothing previously asserted these two
agree — this is exactly the mechanism that made the After School fix (a
`payrollRoleCostSourceData.ts`-only correction) invisible to the tree, which
never reads that file at all.

Per the product owner's instruction to implement the previously-offered
recommendation, a permanent parity gate was added —
`scripts/validate-v10-rc2-5-gate9-org-tree-payroll-parity.ts`
(`npm run validate:v10-rc2-5-gate9`).

**Design choice, deliberate:** rather than hand-writing a second list
mapping tree-node IDs to payroll roleIds (exactly the parallel-list pattern
behind this session's Counselor and After School bugs), the tree itself was
made self-describing. `OrgTreeNode` gained an optional `payrollRoleId?:
string` field; `getExistingRoleHeadcount()` in `executiveOrgDesignModel.ts`
now tags every node it produces with the `roleId` it read. The validator
walks the real tree output, collects every tagged node, and compares its
`headcountValue` against `calculateFopag()`'s live record for that exact
`roleId`/year — a purely mechanical check with no hardcoded role list to
maintain or let drift. `ORG_DESIGN_SCENARIO_OPTION_BY_ID` (the
scenario→orgDesignOptionId map) was also exported for the same reason,
rather than duplicated in the validator.

Deliberately excluded from the check, by construction rather than by an
exclusion list: the EY/LS/HS Counselor nodes (`getCounselorHeadcount()`/
`getHsCounselorHeadcount()`, hardcoded to 1, intentionally not
payroll-wired — documented in `orgDesignLogic.md`) and the MS Counselor /
unresolved-allocation nodes (a derived increment, not a direct
`role.headcount[year]` read, per this session's earlier Counselor
correction) — none of these call `getExistingRoleHeadcount()`, so none set
`payrollRoleId`, so none enter the comparison. This is correct: comparing
them against the raw aggregate would be the wrong check, not a stricter one.

**Coverage:** all 3 scenarios × 10 years × both opening packages (verified,
not merely assumed, that baseline non-teaching roles don't vary by
openingPackageId — confirmed by reading `payrollAdapter.ts` section 1's
`resolveHeadcount()`, which takes no package argument — and then exercised
both packages anyway as a belt-and-suspenders proof). 1,368 node-level
comparisons, 0 mismatches — confirming the After School fix earlier this
session fully closed the only divergence that existed. A third assertion
guards against the check silently passing with zero comparisons if a future
refactor ever stops tagging nodes (asserts `comparisons >= 900`).

**Validation:** `npx tsc --noEmit` clean; `npm run validate:v10-rc2-5-gate9`
3/3 pass (1,368 comparisons, 0 mismatches); `npm run validate:v10-rc2-5-gate7`
52/52 pass, unaffected; `npm run validate:v10-x2t` 109/110 pass, unaffected
(same single pre-existing failure); `npm run build` clean; `git diff --check`
clean.
