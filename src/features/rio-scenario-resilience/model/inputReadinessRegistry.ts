...
export const INPUT_READINESS_REGISTRY: InputReadinessRegistry = {
  ...
  selected_org_design_option: {
    ...
    status: "structural_only",
    currentSource: "data/orgDesignStructure.ts orgDesignStructure; scenarioOfferRoleActivationSourceContract",
    sourceOwnership: "simulator_owned",
    canUseForCalculation: false,
    blockingReason: "missing_mapping",
    requiredNextAction: "Create machine-readable Scenario Offer role activation records before binding options to role mappings.",
    dependsOn: [],
    futureArtifactOrFile: "future Scenario Offer role activation mapping table",
  },
  baseline_role_set: {
    ...
    status: "needs_mapping",
    currentSource: "data/orgDesignStructure.ts baselineRoleSet; reference-only existing payroll role sources",
    sourceOwnership: "reference_only",
    canUseForCalculation: false,
    blockingReason: "missing_mapping",
    requiredNextAction: "Map approved existing roles into ValidatedRoleCostMap.",
  ...
  org_design_extension_roles: {
    ...
    status: "blocked",
    currentSource: "data/orgDesignStructure.ts additionalRoleIds; orgDesignCompensationAliases",
    sourceOwnership: "simulator_owned",
    canUseForCalculation: false,
    blockingReason: "missing_mapping",
    requiredNextAction: "Map Scenario Offer role activations to display roles, then validate every staffing field before financial use.",
    dependsOn: ["selected_org_design_option"],
    futureArtifactOrFile: "future Scenario Offer role activation mapping table",
  },
  validated_role_cost_map: {
  ...
  teaching_compensation_data: {
    ...
    status: "needs_mapping",
    currentSource: "reference-only src/constants/teaching.ts; payrollAdapterContract.ts PayrollRoleSource",
    sourceOwnership: "reference_only",
    canUseForCalculation: false,
    blockingReason: "missing_mapping",
    requiredNextAction: "Map only approved teaching compensation records.",
    dependsOn: [],
    futureArtifactOrFile: "future payroll role-cost and staffing mapping table",
  },
  staffing_rules: {
