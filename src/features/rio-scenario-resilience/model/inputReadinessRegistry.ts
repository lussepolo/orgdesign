...
export const INPUT_READINESS_REGISTRY: InputReadinessRegistry = {
  ...
  annual_tuition_adjustment_assumptions: {
    ...
    status: "confirmed",
    currentSource: "tuitionGrowth.ts V10_TUITION_SOURCE (v10 workbook PnL row 9, Reajuste Serviços; V10-F2, 2026-07-27)",
    sourceOwnership: "simulator_owned",
    canUseForCalculation: false,
    blockingReason: "missing_formula_definition",
    requiredNextAction: "Confirmed. Receita calculation remains blocked by enrollment-to-tuition grade mapping and formula approval.",
    dependsOn: [],
    futureArtifactOrFile: "future Receita calculation design",
    notes: "5.9% per year, compounded annually, starting 2029 (6.0% one-time 2028 rate per v10 PnL row 9 is NOT applied — stored tuition figures are already explicit 2028 values). Base year 2028 factor = 1.0. Basis: annual gross contract value. Supersedes the prior 8% convention.",
  },
  occupancy_input: {
  ...
  baseline_role_set: {
    ...
    status: "needs_mapping",
    currentSource: "payrollRoleCostSourceData.ts PAYROLL_ROLE_COST_SOURCE_RECORDS — 25 cost-ready role records (9 Leadership + 12 Backoffice + 4 Specialists) extracted from src/constants/leadership.ts (corrected 2026-07-30: was 26/8/12/6 before After School Coordinator moved from Specialists to Leadership and hs_pool was deleted as dead config). Role cost values (grossMonthly, laborChargesMonthly, benefitsMonthly) are Finance-validated per Luciana confirmation (2026-06-03). Source stores raw monthly cost components and headcount progressions.",
    sourceOwnership: "mapped_existing_source",
    canUseForCalculation: false,
    blockingReason: "missing_mapping",
