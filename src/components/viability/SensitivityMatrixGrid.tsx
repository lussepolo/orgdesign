...
export default function SensitivityMatrixGrid({
  ...
  return (
    <Card
      ...
      subtitle={t("sensitivityMatrixSubtitle")
        .replace("{metric}", viewModel.metric)
        .replace("{row}", formatVariableLabel(viewModel.rowVariable))
        .replace("{column}", formatVariableLabel(viewModel.columnVariable))}
    >
          ...
          <thead>
            <tr>
              ...
              {viewModel.columnOptions.map((option) => (
                ...
                >
                  {t("sensitivityMatrixColumnValue")}
                  <div className="mt-1 text-sm font-bold normal-case tracking-normal text-slate-900">
                    {option.label}
                  </div>
                </th>
                  ...
                  <td key={cell.runLabel}>
                    ...
                    >
                      ...
                      <div className="mt-2 grid gap-2 text-[11px] text-slate-500">
                        <div>
                          <span className="font-bold text-slate-700">{t("sensitivityMatrixRowLabel")}</span> {cell.rowValue}
                        </div>
                        <div>
                          <span className="font-bold text-slate-700">{t("sensitivityMatrixColumnLabel")}</span> {cell.columnValue}
                        </div>
                      </div>
                      <div className="mt-3 flex items-center justify-between gap-3">
                        ...
                        <div className="rounded-full border border-slate-200 px-2 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">
                          {t("sensitivityMatrixDirectionalBadge")}
                        </div>
                      </div>
                    </div>
                  </td>
